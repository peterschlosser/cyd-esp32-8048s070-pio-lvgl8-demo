...
### 2. Prepare PlatformIO Project

1. Create a new PlatformIO project using the `Sunton ESP32-S3` Board type and `Arduino` Framework

2. Save the VSCode workspace

3. Install the LvGL 8.4.0 Library within PlatformIO, or any newer version below 9.0.0.

4. Install the Lovyan GFX 1.2.7 (or newer) Library within PlatformIO
...
2. optional: enable GNU++17 C++ compiler, optimize output for size, and set ESP32 log level to INFO:
...
3. enable storing the `lv_conf.h` file in the project `lib` folder, and protect it from being erased by a "Full Clean" operation:
...
4. save the changes to `platformio.ini`
...
4. change the type of memory from which buffers are allocated to PSRAM:
	...
	#endif
	```
5. enable `LV_TICK_CUSTOM` so we eliminate the need to call `lv_tick_inc()` from our main `loop()` and simplify our code:
	```cpp
	#define LV_TICK_CUSTOM 1
	```
6. optional: enable the logging of messages in case something goes wrong
...
1. to the top of main.cpp, add the #includes of the LovyanGFX driver:
	```cpp
...
4. below the "setup started." log entry within `setup()`, add the following lines to setup GFX and use of the display:
	```cpp
...
1. to the top of `main.cpp`, add the #includes of the LvGL library and widgets demo:
	```cpp
	#include <lvgl.h>
	```
...
7. initialize and register the input driver with LvGL.  add the following lines to the bottom of `setup()`:
	```cpp
...
8. prepare the main `loop()` to drive LvGL and do its thing.  add the following lines to the `loop()` function:
	```cpp
	lv_timer_handler();
...
### 8. Prepare Widgets Demo

these steps are unnecessary if our project does not use the widgets demo.

1. modify `lv_conf.h` and enable the LvGL widgets demo as our example display content:
	```cpp
	#define LV_USE_DEMO_WIDGETS 1
	```

2. to the top of `main.cpp`, add the #include of the LvGL widgets demo:
	```cpp
	#include <demos/lv_demos.h>
	```
	
3. as of this writing, when using LvGL 8.4.0, the `demos` folder containing the widgets demo must be copied in order for it to be compiled and correctly linked to the firmware binary.  Copy the whole `demos` folder from: `.pio/libdeps/sunton_s3/lvgl/demos` into the `src` folder: `.pio/libdeps/sunton_s3/lvgl/src` such that when complete, there is a copy of the `demos` folder at: `.pio/libdeps/sunton_s3/lvgl/src/demos`

4. modify `main.cpp` to draw the demo content.  add the following lines to the bottom of `setup()`:
	```cpp
	lv_demo_widgets();
	ESP_LOGI(TAG, "gui started.");
	```

5. **Test Widgets Demo.** compile and upload the firmware to our device.  we should observe the "gui started" message in the serial monitor and a rich demo of widgets on the display.

Note: be aware platformio may update the libdeps (library dependencies) from time to time, and when it does, it may remove the copied demos folder.  When building, if we get the error `undefined reference to 'lv_demo_widgets'` we may need to repeat the changes made to the LvGL library.
